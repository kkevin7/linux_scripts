...
echo "=== Instalador NVIDIA para Debian 13 (MSI RTX 4070 Ventus) - Entorno XFCE ==="
...
# 6. Configurar LightDM para sesión con Xorg
echo "[6/9] Asegurando que LightDM use Xorg..."
sudo mkdir -p /etc/lightdm/lightdm.conf.d
cat <<EOF | sudo tee /etc/lightdm/lightdm.conf.d/20-xorg.conf
[Seat:*]
# Forzar uso de Xorg (desactivar Wayland - aunque LightDM no lo usa por defecto)
display-setup-script=/usr/bin/nvidia-xconfig
EOF
